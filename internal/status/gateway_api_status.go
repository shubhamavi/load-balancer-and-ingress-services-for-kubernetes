...
type UpdateGtwApiGWStatusConditionOptions struct {
...
func UpdateGtwApiGatewayStatusAddress(options []UpdateOptions, bulk bool) {
	...
	gatewayMap := getGtwApiGateways(gatewaysToUpdate, bulk)
	for _, option := range updateGWOptions {
		...
		if gw := gatewayMap[option.IngSvc]; gw != nil {
			...
			gwStatus.Addresses = []gtwapiv1alpha1.GatewayAddress{{
				Value: option.Vip,
				Type:  gtwapiv1alpha1.IPAddressType,
			}}

			// when statuses are synced during bootup
			InitializeGtwApiGatewayConditions(gwStatus, &gw.Spec, true)
			UpdateGtwApiGatewayStatusGWCondition(option.Key, gwStatus, &UpdateGtwApiGWStatusConditionOptions{
				Type:   "Ready",
				Status: metav1.ConditionTrue,
			})
			UpdateGtwApiGatewayStatusObject(option.Key, gw, gwStatus)
		}
	}
...
func getGtwApiGateways(gwNSNames []string, bulk bool, retryNum ...int) map[string]*gtwapiv1alpha1.Gateway {
	retry := 0
	gwMap := make(map[string]*gtwapiv1alpha1.Gateway)
	if len(retryNum) > 0 {
	...
	if bulk {
		gwList, err := lib.GetGatewayAPIClientset().NetworkingV1alpha1().Gateways("").List(context.TODO(), metav1.ListOptions{})
		if err != nil {
			...
			if strings.Contains(err.Error(), utils.K8S_ETIMEDOUT) {
				return getGtwApiGateways(gwNSNames, bulk, retry+1)
			}
		} else {
	...
	for _, namespaceName := range gwNSNames {
		...
		gw, err := lib.GetGatewayAPIClientset().NetworkingV1alpha1().Gateways(nsNameSplit[0]).Get(context.TODO(), nsNameSplit[1], metav1.GetOptions{})
		if err != nil {
			utils.AviLog.Warnf("Could not get the gateway object for UpdateStatus: %s", err)
			// retry get if request timeout
			if strings.Contains(err.Error(), utils.K8S_ETIMEDOUT) {
...
func DeleteGtwApiGatewayStatusAddress(key string, svcMetadataObj avicache.ServiceMetadataObj) error {
	gwNSName := strings.Split(svcMetadataObj.Gateway, "/")
	gw, err := lib.GetGatewayAPIClientset().NetworkingV1alpha1().Gateways(gwNSName[0]).Get(context.TODO(), gwNSName[1], metav1.GetOptions{})
	if err != nil {
	...
	gwStatus := gw.Status.DeepCopy()
	gwStatus.Addresses = []gtwapiv1alpha1.GatewayAddress{}
	UpdateGtwApiGatewayStatusGWCondition(key, gwStatus, &UpdateGtwApiGWStatusConditionOptions{
		Type:   "Pending",
		Status: metav1.ConditionTrue,
		Reason: "virtualservice deleted/notfound",
	})
	UpdateGtwApiGatewayStatusObject(key, gw, gwStatus)

	utils.AviLog.Infof("key: %s, msg: Successfully reset the address status of gateway: %s", key, svcMetadataObj.Gateway)
	return nil
}

func DeleteGtwApiStatus(key string, svcMetadataObj avicache.ServiceMetadataObj) error {
	gwNSName := strings.Split(svcMetadataObj.Gateway, "/")
	gw, err := lib.GetGatewayAPIClientset().NetworkingV1alpha1().Gateways(gwNSName[0]).Get(context.TODO(), gwNSName[1], metav1.GetOptions{})
	if err != nil {
		utils.AviLog.Warnf("key: %s, msg: there was a problem in resetting the gateway address status: %s", key, err)
...
func UpdateGtwApiGatewayStatusGWCondition(key string, gwStatus *gtwapiv1alpha1.GatewayStatus, updateStatus *UpdateGtwApiGWStatusConditionOptions) {
	utils.AviLog.Debugf("key: %s, msg: Updating Gateway status gateway condition %v", key, utils.Stringify(updateStatus))
	InitializeGtwApiGatewayConditions(gwStatus, nil, false)
	...
	UpdateGtwApiGatewayStatusListenerConditions(key, gwStatus, "", &UpdateGtwApiGWStatusConditionOptions{
		Type:   "Ready",
...
func UpdateGtwApiGatewayStatusListenerConditions(key string, gwStatus *gtwapiv1alpha1.GatewayStatus, portString string, updateStatus *UpdateGtwApiGWStatusConditionOptions) {
...
func UpdateGtwApiGatewayStatusObject(key string, gw *gtwapiv1alpha1.Gateway, updateStatus *gtwapiv1alpha1.GatewayStatus, retryNum ...int) {
	retry := 0
	if len(retryNum) > 0 {
		retry = retryNum[0]
		if retry >= 5 {
			utils.AviLog.Errorf("key: %s, msg: UpdateGtwApiGatewayStatusObject retried 5 times, aborting", key)
			return
		}
	}
	...
	if len(updateStatus.Addresses) > 0 && updateStatus.Addresses[0].Value != "" {
		lib.CheckAndSetGtwApiGatewayFinalizer(gw)
	} else {
		lib.RemoveGtwApiGatewayFinalizer(gw)
	}

	if compareGtwApiGatewayStatuses(&gw.Status, updateStatus) {
		return
	}
	...
	// TODO: Patch call to status not working.
	gw.Status = *updateStatus
	newGw, err := lib.GetGatewayAPIClientset().NetworkingV1alpha1().Gateways(gw.Namespace).UpdateStatus(context.TODO(), gw, metav1.UpdateOptions{})
	// newGw, err := lib.GetGatewayAPIClientset().NetworkingV1alpha1().Gateways(gw.Namespace).Patch(context.TODO(), gw.Name, types.MergePatchType, patchPayload, metav1.PatchOptions{}, "status")
	if err != nil {
		utils.AviLog.Warnf("msg: %d there was an error in updating the gateway status: %+v", retry, err)
		updatedGW, err := lib.GetGatewayAPIClientset().NetworkingV1alpha1().Gateways(gw.Namespace).Get(context.TODO(), gw.Name, metav1.GetOptions{})
		if err != nil {
			utils.AviLog.Warnf("gateway not found %v", err)
			return
		}
		UpdateGtwApiGatewayStatusObject(key, updatedGW, updateStatus, retry+1)
	}
...
func InitializeGtwApiGatewayConditions(gwStatus *gtwapiv1alpha1.GatewayStatus, gwSpec *gtwapiv1alpha1.GatewaySpec, gwReady bool) {
	...
	gwPortMap := make(map[gtwapiv1alpha1.PortNumber][]metav1.Condition)
	for _, listenerStatus := range gwStatus.Listeners {
		gwPortMap[listenerStatus.Port] = listenerStatus.Conditions
	}

	var listenerStatuses []gtwapiv1alpha1.ListenerStatus
	for _, listener := range gwSpec.Listeners {
		if val, ok := gwPortMap[gtwapiv1alpha1.PortNumber(listener.Port)]; ok {
			listenerStatuses = append(listenerStatuses, gtwapiv1alpha1.ListenerStatus{
				Port:       listener.Port,
				Conditions: val,
			})
		} else {
			...
			listenerStatuses = append(listenerStatuses, gtwapiv1alpha1.ListenerStatus{
				...
				Conditions: []metav1.Condition{{
					...
					Reason:             string(gtwapiv1alpha1.GatewayReasonNotReconciled),
				}},
			})
		}
	}
...
func compareGtwApiGatewayStatuses(old, new *gtwapiv1alpha1.GatewayStatus) bool {
