...
func (rest *RestOperations) AviDSBuild(ds_meta *nodes.AviHTTPDataScriptNode, cache_obj *avicache.AviDSCache, key string) *utils.RestOp {
	...
	vsdatascriptset := avimodels.VSDataScriptSet{
		CreatedBy: &cr,
		Datascript: []*avimodels.VSDataScript{{
			Evt:    &ds_meta.Evt,
			Script: &ds_meta.Script,
		}},
		Name:          &ds_meta.Name,
	...
	utils.AviLog.Debugf("key: %s, msg: ds Restop %v DatascriptData %v\n", key, utils.Stringify(rest_op), utils.Stringify(ds_meta))
	return &rest_op
}
...
func (rest *RestOperations) AviDSDel(uuid string, tenant string, key string) *utils.RestOp {
	...
	utils.AviLog.Infof("key: %s, msg: DS DELETE Restop %v \n", key, utils.Stringify(rest_op))
	return &rest_op
}
...
func (rest *RestOperations) AviDSCacheAdd(rest_op *utils.RestOp, vsKey avicache.NamespaceName, key string) error {
	...
	for _, resp := range resp_elems {
		...
		ds_cache_obj := avicache.AviDSCache{
			Name:       name,
		...
		if ok {
			...
			vs_cache_obj.AddToDSKeyCollection(k)
			utils.AviLog.Infof("key: %s, msg: added VS cache key during datascriptset update %v val %v\n", key, vsKey, utils.Stringify(vs_cache_obj))
		}
		utils.AviLog.Infof("key: %s, msg: added Datascriptset cache k %v val %v\n", key, k, utils.Stringify(ds_cache_obj))
	}
