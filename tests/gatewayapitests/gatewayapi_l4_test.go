...
import (
	...
	gtwapifake "sigs.k8s.io/gateway-api/pkg/client/clientset/versioned/fake"
...
var GtwAPIClient *gtwapifake.Clientset
var ctrl *k8s.AviController
var CRDClient *crdfake.Clientset

func TestMain(m *testing.M) {
	os.Setenv("GATEWAY_API", "true")
	...
	GtwAPIClient = gtwapifake.NewSimpleClientset()
...
func (gw FakeGateway) Gateway() *gatewayapi.Gateway {
	var fakeListeners []gatewayapi.Listener
	for _, listener := range gw.Listeners {
		fakeListeners = append(fakeListeners, gatewayapi.Listener{
			Port:     listener.Port,
			Protocol: gatewayapi.ProtocolType(listener.Protocol),
			Routes: gatewayapi.RouteBindingSelector{
				Kind: "services",
	...
	if gw.IPAddress != "" {
		gateway.Spec.Addresses = []gatewayapi.GatewayAddress{{
			Type:  gatewayapi.IPAddressType,
			Value: gw.IPAddress,
		}}
	}
...
func SetupGateway(t *testing.T, gwname, namespace, gwclass string) {
	gateway := FakeGateway{
		...
		Listeners: []FakeGWListener{{
			...
			Protocol: "TCP",
			Labels: map[string]string{
				lib.GtwApiGatewayNameLabelKey:      gwname,
				lib.GtwApiGatewayNamespaceLabelKey: namespace,
			},
		}},
	}

	gwCreate := gateway.Gateway()
	if _, err := lib.GetGatewayAPIClientset().NetworkingV1alpha1().Gateways(namespace).Create(context.TODO(), gwCreate, metav1.CreateOptions{}); err != nil {
		t.Fatalf("error in adding Gateway: %v", err)
	}
}

func TeardownGateway(t *testing.T, gwname, namespace string) {
	if err := lib.GetGatewayAPIClientset().NetworkingV1alpha1().Gateways(namespace).Delete(context.TODO(), gwname, metav1.DeleteOptions{}); err != nil {
...
func (gwclass FakeGWClass) GatewayClass() *gatewayapi.GatewayClass {
	...
	if gwclass.InfraSetting != "" {
		gatewayclass.Spec.ParametersRef = &gatewayapi.ParametersReference{
			Group: lib.AkoGroup,
...
func SetupGatewayClass(t *testing.T, gwclassName, controller, infraSetting string) {
	...
	if _, err := lib.GetGatewayAPIClientset().NetworkingV1alpha1().GatewayClasses().Create(context.TODO(), gwClassCreate, metav1.CreateOptions{}); err != nil {
		t.Fatalf("error in adding GatewayClass: %v", err)
	}
}

func TeardownGatewayClass(t *testing.T, gwClassName string) {
	if err := lib.GetGatewayAPIClientset().NetworkingV1alpha1().GatewayClasses().Delete(context.TODO(), gwClassName, metav1.DeleteOptions{}); err != nil {
		t.Fatalf("error in deleting GatewayClass: %v", err)
	}
}

func SetupGtwApiLBService(t *testing.T, svcname, namespace, gwname, gwnamespace string) {
	svc := integrationtest.FakeService{
		Name:      svcname,
		Namespace: namespace,
		Labels: map[string]string{
			lib.GtwApiGatewayNameLabelKey:      gwname,
			lib.GtwApiGatewayNamespaceLabelKey: gwnamespace,
		},
...
func TestGatewayAPIBestCase(t *testing.T) {
	...
	SetupGatewayClass(t, gwClassName, lib.GtwApiAviGatewayController, "")
	SetupGateway(t, gatewayName, ns, gwClassName)

	SetupGtwApiLBService(t, "svc", ns, gatewayName, ns)

	g.Eventually(func() string {
		gw, _ := GtwAPIClient.NetworkingV1alpha1().Gateways(ns).Get(context.TODO(), gatewayName, metav1.GetOptions{})
...
func TestGatewayAPINamingConvention(t *testing.T) {
	// create gwclass, create gw, create 1svc
...
func TestGatewayAPIWithStaticIP(t *testing.T) {
	// create gwclass, create gw, create 1svc
	...
	gateway := FakeGateway{
		...
		Listeners: []FakeGWListener{{
			...
			Protocol: "TCP",
			Labels: map[string]string{
				lib.GtwApiGatewayNameLabelKey:      gatewayName,
				lib.GtwApiGatewayNamespaceLabelKey: ns,
			},
		}},
	}
	gwCreate := gateway.Gateway()
	if _, err := lib.GetGatewayAPIClientset().NetworkingV1alpha1().Gateways(ns).Create(context.TODO(), gwCreate, metav1.CreateOptions{}); err != nil {
		t.Fatalf("error in adding Gateway: %v", err)
	}
...
func TestGatewayAPIWrongControllerGWClass(t *testing.T) {
	...
	if _, err := lib.GetGatewayAPIClientset().NetworkingV1alpha1().GatewayClasses().Update(context.TODO(), gwclassUpdate, metav1.UpdateOptions{}); err != nil {
...
func TestGatewayAPIWrongClassMappingInGateway(t *testing.T) {
	// create gwclass, gw
	...
	gwUpdate := FakeGateway{
		...
		Listeners: []FakeGWListener{{
			...
			Labels: map[string]string{
				lib.GtwApiGatewayNameLabelKey:      "BADGATEWAY",
				lib.GtwApiGatewayNamespaceLabelKey: ns,
			},
		}},
	}.Gateway()
	gwUpdate.ResourceVersion = "2"
	if _, err := lib.GetGatewayAPIClientset().NetworkingV1alpha1().Gateways(ns).Update(context.TODO(), gwUpdate, metav1.UpdateOptions{}); err != nil {
...
func TestGatewayAPIProtocolChangeInService(t *testing.T) {
...
func TestGatewayAPIPortChangeInService(t *testing.T) {
...
func TestGatewayAPILabelUpdatesInService(t *testing.T) {
...
func TestGatewayAPILabelUpdatesInGateway(t *testing.T) {
	// correct labels, label mismatch, correct labels, delete labels
	g := gomega.NewGomegaWithT(t)
...
func TestGatewayAPIGatewayListenerPortUpdate(t *testing.T) {
	// svc/tcp/8081
...
func TestGatewayAPIGatewayListenerProtocolUpdate(t *testing.T) {
	// svc/tcp/8080
...
func TestGatewayAPIMultiGatewayServiceUpdate(t *testing.T) {
	...
	SetupGtwApiLBService(t, "svc", ns, gateway1Name, ns)
	...
	svcUpdate := integrationtest.FakeService{
		...
		Namespace: ns,
		Labels: map[string]string{
			lib.GtwApiGatewayNameLabelKey:      gateway2Name,
			lib.GtwApiGatewayNamespaceLabelKey: ns,
		},
		Type:         corev1.ServiceTypeLoadBalancer,
		ServicePorts: []integrationtest.Serviceport{{PortName: "foo1", Protocol: corev1.ProtocolTCP, PortNumber: 8081, TargetPort: 8081}},
...
func TestGatewayAPIEndpointDeleteCreate(t *testing.T) {
...
func TestGatewayAPIWithInfraSettingStatusUpdates(t *testing.T) {
	...
	SetupGatewayClass(t, gwClassName, lib.GtwApiAviGatewayController, settingName)
	SetupGateway(t, gatewayName, ns, gwClassName)
	SetupGtwApiLBService(t, "svc", ns, gatewayName, ns)
...
func TestGatewayAPIInfraSettingDelete(t *testing.T) {
...
func TestGatewayAPIInfraSettingChangeMapping(t *testing.T) {
	...
	SetupGatewayClass(t, gwClassName, lib.GtwApiAviGatewayController, settingName1)
	SetupGateway(t, gatewayName, ns, gwClassName)
	SetupGtwApiLBService(t, "svc", ns, gatewayName, ns)
	...
	gwClassUpdate := (FakeGWClass{
		Name:         gwClassName,
		Controller:   lib.GtwApiAviGatewayController,
